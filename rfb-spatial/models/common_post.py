...
class DConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True, dilation = 2):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.dconv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False, dilation=dilation)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
    ...
    def forward_fuse(self, x):
        return self.act(self.__delattr__conv(x))
...
class ARF(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.branch0 = nn.Sequential(
            BasicConv2d(in_channel, out_channel, 1, dilation=1),
        )
        ...
        #     BasicConv2d(in_channel, out_channel, 1),
        #     # BasicConv2d(out_channel, out_channel, kernel_size=(1, 1), padding=(0, 1)),
        #     # BasicConv2d(out_channel, out_channel, kernel_size=(1, 1), padding=(1, 0)),
    ...
    def forward(self, x):
        ...
        #x_cat = self.conv_cat(torch.cat((x0), dim=1))
...
class C3D(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv1 = DConv(c1, c_, 1, 1, dilation=1)
        self.cv2 = DConv(c1, c_, 1, 1, dilation=2)
        self.cv3 = DConv(2 * c_, c2, 1, dilation=3)  # act=FReLU(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
