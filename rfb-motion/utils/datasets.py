...
class LoadClipsAndLabels(Dataset):
    ...
    #def calculate_dense_optical_flow(prev_frame, next_frame):

    # def calculate_dense_optical_flow(self, prev_frame, next_frame):
    ...
    # #def stabilize_frames(frame1, frame2, frame3):
    # def stabilize_frames(self, frame1, frame2, frame3):
    #     # Convert frames to grayscale
    #     #print(frame1.shape)
    ...
    #     matches2to3 = bf.match(descriptors2, descriptors3)
    ...
    #     src_pts = np.float32([keypoints1[m.queryIdx].pt for m in matches1to2]).reshape(-1, 1, 2)
    #     dst_pts = np.float32([keypoints2[m.trainIdx].pt for m in matches1to2]).reshape(-1, 1, 2)
        
    #     src_pts_2to3 = np.float32([keypoints2[m.queryIdx].pt for m in matches2to3]).reshape(-1, 1, 2)
    #     dst_pts_2to3 = np.float32([keypoints3[m.trainIdx].pt for m in matches2to3]).reshape(-1, 1, 2)
        
    #     # Calculate Homography for the first pair
    #     H, _ = cv2.findHomography(src_pts, dst_pts, cv2.RANSAC, 5.0)
        
    #     # Apply Homography to the second pair
    #     stabilized_frame2 = cv2.warpPerspective(frame2, H, (frame2.shape[1], frame2.shape[0]))
        
    #     # Calculate Homography for the second pair
    #     H_2to3, _ = cv2.findHomography(src_pts_2to3, dst_pts_2to3, cv2.RANSAC, 5.0)
        
    #     # Apply Homography to the second frame and then the third frame
    #     stabilized_frame3 = cv2.warpPerspective(stabilized_frame2, H_2to3, (frame3.shape[1], frame3.shape[0]))
        
    #     return stabilized_frame2, stabilized_frame3
    
    # def calculate_optical_flow(self, frame1, frame2, frame3):
    #     # Convert frames to grayscale
    #     gray1 = cv2.cvtColor(frame1, cv2.COLOR_BGR2GRAY)
    #     gray2 = cv2.cvtColor(frame2, cv2.COLOR_BGR2GRAY)
    #     gray3 = cv2.cvtColor(frame3, cv2.COLOR_BGR2GRAY)
    ...
    #     pts1 = cv2.goodFeaturesToTrack(gray1, maxCorners=100, qualityLevel=0.3, minDistance=7, blockSize=7)

    #     if pts1 is None:
    #         return np.zeros_like(frame3)
    #     else:
    #         pts2, status, err = cv2.calcOpticalFlowPyrLK(gray1, gray2, pts1, None, **lk_params)
    ...
    #         # Calculate optical flow from frame2 to frame3
    #         pts3, status, err = cv2.calcOpticalFlowPyrLK(gray2, gray3, pts2, None, **lk_params)
    ...
    #         pts3 = pts3[status.flatten() == 1]
            
    #         output_frame3 = np.zeros_like(frame3)

    #         for  pt3 in  pts3:
    #             x3, y3 = np.round(pt3).astype(int).ravel()

    #             # Ensure points are within the frame bounds

    #             x3 = np.clip(x3, 0, frame3.shape[1] - 1)
    #             y3 = np.clip(y3, 0, frame3.shape[0] - 1)

    #             # Set RGB values at the points

    #             output_frame3[y3, x3] = frame3[y3, x3]
        


        # return output_frame3
